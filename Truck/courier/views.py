...
def available_job_page(request, id):
    ...
    if request.method == 'POST':
        ...
        courier_name = job.courier.user.get_full_name()  # Get the courier's full name
        job_created_time = job.created_at.strftime('%Y-%m-%d %H:%M:%S')
        
        # Render the email body from an HTML template
        email_subject = 'Courier On The Way'
        email_body = render_to_string('emails/courier_on_the_way.html', {
            'customer_name': customer_name,
            'job': job,
            'courier_name': courier_name,  # Pass the courier's name to the template
...
def archived_jobs_page(request):
    ...
    return render(request, 'courier/archived-jobs.html',{
        "jobs": jobs
    })
...
def profile_page(request):
    ...
    total_earnings = round(sum(job.price for job in completed_jobs) * 0.8, 2)  # 80% earning
...
def payout_method_page(request):
    ...
    return render(request, 'courier/payout-method.html',{
